...
def pick(paragraphs, select, k):
    ...
    for paragraph in paragraphs:
        if select(paragraph) == True:
            paragraph_suit += [paragraph]
            index += 1
    return paragraph_suit[k] if k < index else ''
...
def about(subject):
    ...
    def select_word(sentence):
        s = split(remove_punctuation(sentence))
        for word in s:
            word = lower(word)
            if word in subject:
                return True
...
def autocorrect(typed_word, word_list, diff_function, limit):
    ...
    for word in word_list:
        diff = diff + [diff_function(typed_word,word,limit)]
    if all([num > limit for num in diff]):
        return typed_word
...
def furry_fixes(typed, source, limit):
    ...
    def func(a,b,total):
        ...
        if a[0] == b[0]:
            return func(a[1:],b[1:],total)
        else:
            return func(a[1:],b[1:],total + 1) + 1
...
def minimum_mewtations(typed, source, limit):
    ...
    if not typed and not source:
        return 0
    elif not typed or not source: # Base cases should go here, you may add more base cases as needed.
        # BEGIN
        "*** YOUR CODE HERE ***"
...
def report_progress(typed, source, user_id, upload):
    ...
    for i in range(len(typed)):
        if typed[i] == source[i]:
            ...
            break   #遇到第一个就停止
    upload({'id' : user_id,'progress': progress/len(source)})
    return progress / len(source)
    # END PROBLEM 8
...
def time_per_word(words, timestamps_per_player):
    ...
    # BEGIN PROBLEM 9
    times = [[tpp[0][i+1]-tpp[0][i] for i in range(len(tpp[0])-1)]]
    for k in range(1,len(tpp)):
        times = times + [[tpp[k][j+1]-tpp[k][j] for j in range(len(tpp[k])-1)]]
    # END PROBLEM 9
    ...
    for j in word_indices:
        ...
        for i in player_indices:
            if times[i][j] < min_time:
                min_time = times[i][j]
                fastest_player = i
        result[fastest_player].append(words[j])
    return result
    # END PROBLEM 10
