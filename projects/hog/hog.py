...
def sus_update(num_rolls, player_score, opponent_score, dice=six_sided):
    ...
    return result
    #刚刚problem5一直过不了，结果发现是这里写复杂了，没必要反复调用函数，注意这里把步骤拆解成了很多简单的函数后就多调用前面的函数
    # END PROBLEM 4
...
def max_scoring_num_rolls(dice=six_sided, samples_count=1000):
    ...
    for i in range(1,11):
        ...
        if avg > max_avg:
            ...
            j = i
    return j
    #比较最大值的时候最好不要反复调用函数 用赋值的方法把最大值固定下来
    # END PROBLEM 9
...
def boar_strategy(score, opponent_score, threshold=11, num_rolls=6):
    ...
    """
    # BEGIN PROBLEM 10
    m = boar_brawl(score, opponent_score)
    if m >= threshold:
        return 0
...
def sus_strategy(score, opponent_score, threshold=11, num_rolls=6):
    ...
    # BEGIN PROBLEM 11
    m = sus_update(0, score, opponent_score)
